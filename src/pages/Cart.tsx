...
export function Cart() {
  ...
  const handleUpdateQuantity = (productId: string, colorName: string, currentQty: number, delta: number, maxStock: number) => {
    dispatch(updateQty({ productId, selectedColor: colorName, quantity: currentQty + delta, maxStock }));
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          {items.map((item) => {
            ...
            const maxStock = productInfo?.stockByColor[item.selectedColor.name] || 0;
                  ...
                  <div className="flex items-center justify-between mt-4 sm:mt-0">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center border border-[var(--border)] rounded-md bg-[var(--surface)]">
                        <button
                          onClick={() => handleUpdateQuantity(item.productId, item.selectedColor.name, item.quantity, -1, maxStock)}
                      ...
                      {isOutOfStock && <span className="text-red-500 text-sm font-semibold">Out of stock</span>}
                    </div>
