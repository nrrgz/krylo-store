...
export function ProductDetails() {
  ...
  const handleQuantityChange = (delta: number) => {
    setQuantity((prev) => {
      ...
      return Math.max(1, Math.min(newQty, Math.max(1, stockAvailable)));
    });
  };
    ...
    <div className="container-base py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 mb-24">
            ...
            <div className="grid grid-cols-4 gap-4">
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`aspect-square bg-[var(--surface-2)] rounded-lg flex items-center justify-center text-xs text-gray-400 font-medium border-2 transition-all ${
                    activeImageIndex === idx ? 'border-[var(--primary)]' : 'border-transparent hover:border-[var(--border)]'
                  }`}
                >
                  {img ? (
        ...
        <div className="flex flex-col">
          <div className="mb-6">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-widest mb-2 block">{product.brand}</span>
                    ...
                    <button
                      ...
                      className={`relative w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                        isSelected
                          ...
                          : 'ring-1 ring-[var(--border)] hover:ring-[var(--muted)]'
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-4">
            <div className="flex items-center border border-[var(--border)] rounded-md bg-[var(--surface)]">
              <button
                ...
                disabled={effectiveQuantity <= 1 || isOutOfStock}
              ...
              <div className="w-12 h-12 flex items-center justify-center font-semibold text-gray-900 text-sm">{effectiveQuantity}</div>
              <button
                onClick={() => handleQuantityChange(1)}
                disabled={effectiveQuantity >= stockAvailable || isOutOfStock}
                className="w-10 h-12 flex items-center justify-center text-gray-500 hover:text-[var(--primary)] transition-colors disabled:opacity-50"
              >
                &#43;
              </button>
            ...
            >
              {isOutOfStock
                ...
                : addedSuccess
                  ? 'Added to cart'
                  : 'Add to Cart - $' + (product.price * effectiveQuantity).toFixed(2)}
            </Button>
          </div>

          <div className="text-sm text-gray-500 italic">
            {isOutOfStock
              ? 'This item is currently unavailable in the selected color.'
              : stockAvailable < 5
      ...
      {relatedProducts.length > 0 && (
        <div className="pt-16 border-t border-[var(--border)]">
          <h2 className="text-2xl font-bold tracking-tight text-gray-900 mb-8">Complete Your Setup</h2>
